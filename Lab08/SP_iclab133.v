...
//    CLOCK GATED                //
//-------------------------------//
GATED_OR GATED_graycode(.CLOCK(clk), .SLEEP_CTRL(sleep_graycode),.RST_N(rst_n), .CLOCK_GATED(G_clock_graycode));
GATED_OR GATED_gaddsub (.CLOCK(clk), .SLEEP_CTRL(sleep_addsub),  .RST_N(rst_n), .CLOCK_GATED(G_clock_addsub));
GATED_OR GATED_SMA	   (.CLOCK(clk), .SLEEP_CTRL(sleep_SMA),     .RST_N(rst_n), .CLOCK_GATED(G_clock_SMA));
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		...
		if( n_state == s_idle || (n_state == s_input))
			sleep_graycode <= 0;
		else
			sleep_graycode <= 1;
	end
...
//    INPUT                      //
//-------------------------------//
always @(posedge G_clock_graycode or negedge rst_n) begin
	if(!rst_n)
		mode <= 0;
	else begin
...
generate
for(i=0; i<9 ; i=i+1)begin
	always @(posedge G_clock_addsub or negedge rst_n)begin
		if(!rst_n)begin
			addsub_data_r[i] <= 0;
		end
